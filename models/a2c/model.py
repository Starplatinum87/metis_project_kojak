...
# Create dict of loss results for export
...
def learn(policy,
    ...
    # load_path = "/Users/starplatinum87/Google Drive/DATA_SCIENCE/METIS/METIS_BOOTCAMP/Projects/Project_5_Kojak/sonic/models200/model.ckpt"
    ...
    for update in range(1, total_timesteps//batch_size+1):
        ...
        # Create dictionary of lossvalues results for export to pandas
        ev = explained_variance(values, returns)

        # Append loss data to dictionary
        loss_dict['nupdates'].append(update)
        loss_dict['total_timestamps'].append(update*batch_size)
        loss_dict['fps'].append(fps)
        loss_dict['policy_loss'].append(float(lossvalues[0]))
        loss_dict['policy_entropy'].append(float(lossvalues[2]))
        loss_dict['value_loss'].append(float(lossvalues[1]))
        loss_dict['explained_variance'].append(float(ev))
        loss_dict['time_elapsed'].append(float(tnow - tfirststart))

        if update % log_interval == 0 or update == 1:
            with open("/Users/starplatinum87/Google Drive/DATA_SCIENCE/METIS/METIS_BOOTCAMP/Projects/Project_5_Kojak/sonic/a2c/base/data/v1/loss_dict_"+str(update)+".pkl", 'wb') as f:
        ...
        if update % log_interval == 0 or update == 1:
            ...
            savepath = "/Users/starplatinum87/Google Drive/DATA_SCIENCE/METIS/METIS_BOOTCAMP/Projects/Project_5_Kojak/sonic/a2c/base/models/base/n" + str(update) + "/model.ckpt"
            model.save(savepath)
...
def play(policy, env, slow=False):
    ...
    # Load the model
    load_path = "./models/base/models300/model.ckpt"
    model.load(load_path)
    ...
    while done == False:
        ...
        env.render()
        if slow:
            time.sleep(0.025) # Pause to render playback at a decent speed
